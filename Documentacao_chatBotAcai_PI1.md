...
|RF11|Fazer o Controle de Sessão e Formulários|O sistema deve controlar a sessão do usuário como, verificar se já existe uma sessão ativa, identificar a etapa atual do fluxo (menu, pagamento). Garantir que apenas formulários recentemente solicitados sejam aceitos, desconsiderando respostas de formulários antigos. A sessão deve ser encerrada ao final do pedido, e também se o total for menor ou igual a zero ou seja o usuário cancelou as escolhas, digitando zero na quantidade dos produtos por ele escolhido ou após tempo limite de inatividade.|
